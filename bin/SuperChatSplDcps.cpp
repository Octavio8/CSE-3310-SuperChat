    ...
    struct _SuperChat_message *to)
{
    ...
    to->seqno = (c_ulong)from->seqno;
    to->private_msg_destination_uuid = (c_ulonglong)from->private_msg_destination_uuid;
    to->private_msg = (c_bool)from->private_msg;
    return result;
}
    ...
    struct _SuperChat_user *to)
{
    ...
    to->uuid = (c_ulonglong)from->uuid;
    to->chatroom_idx = (c_ulong)from->chatroom_idx;
    to->graduation_date = (c_ulong)from->graduation_date;
#ifdef OSPL_BOUNDS_CHECK
    if((((c_long)from->captain) >= 0) && (((c_long)from->captain) < 4) ){
        to->captain = (enum _SuperChat_captain_name_enum)from->captain;
    } else {
        OS_REPORT (OS_ERROR, "copyIn", 0,"Member 'SuperChat::user.captain' of type 'captain_name_enum' is out of range.");
        result = OS_C_FALSE;
    }
#else
    to->captain = (enum _SuperChat_captain_name_enum)from->captain;
    ...
    void *_to)
{
    ...
    to->seqno = (::DDS::ULong)from->seqno;
    to->private_msg_destination_uuid = (::DDS::ULongLong)from->private_msg_destination_uuid;
    to->private_msg = (::DDS::Boolean)(from->private_msg != 0);
}
    ...
    void *_to)
{
    ...
    to->uuid = (::DDS::ULongLong)from->uuid;
    to->chatroom_idx = (::DDS::ULong)from->chatroom_idx;
    to->graduation_date = (::DDS::ULong)from->graduation_date;
    to->captain = (::SuperChat::captain_name_enum)from->captain;
}
